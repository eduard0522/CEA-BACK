import {getUserForm,insertAnswer , getFormat} from '../../services/forms/forms.js'
...
export async function getFormatController (req,res){
  try {
    const {user,company} = req.body;
    if(!user || !company){
      return res.status(404).json({status:500, message:'Los campos no pueden estar vacios.'})
    }
    const formUser = await getFormat(user,company);
    if(!formUser)  {
      return res.status(400).json({status:500, message:'Ocurrio un error en el servidor, intente de nuevo mas tarde.'})
    }
    return res.status(200).json({status:200 , data: formUser});
